...
class Step3 extends CI_Controller 
{
    ...
    function index()
    {
        ...
        if ($this->form_validation->run() == TRUE)
        {
            ...
            $config['db']['username'] = $this->input->post('db_username');
            ...
            try 
            {
                ...
                $this->installer->write_app_config();
                $this->installer->db_connect();
                $this->installer->import_schema();
                $this->installer->insert_administrator($this->input->post('email'),$this->input->post('username'), $this->input->post('admin_password'), $this->input->post('first_name'), $this->input->post('last_name'));
                $this->installer->update_db_config($this->input->post('site_name'));
