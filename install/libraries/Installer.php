...
class Installer
{
    ...
    public function write_app_config()
    {
        ...
        $template = file_get_contents(APPPATH . 'assets/config/app.php');
        ...
        $handle = @fopen(CMS_ROOT . 'app.php', 'w+');
        ...
        throw new Exception('Failed to write to ' . CMS_ROOT . 'app.php');
    }


    public function import_schema()
    {
        $file = APPPATH . 'assets/schema/diggitycms.sql';
    ...
    public function insert_administrator($email,$username, $password, $first_name, $last_name)
    {
        ...
        $enpassword = md5($this->encryption_key.$password);
        $this->db_query("INSERT INTO `" . $this->prefix . "user` (`email`,`username`, `password`,`show_password`, `firstname`, `lastname`, `user_group_id`, `enabled`, `activated`, `date_added`) VALUES ('" . $this->db_escape($email) . "','" . $this->db_escape($username) . "', '" . $this->db_escape($enpassword) . "','" . $this->db_escape($show_password) . "' ,'" . $this->db_escape($first_name) . "', '" . $this->db_escape($last_name) . "', 1, 1, 1, '" . date('Y-m-d H:i:s') . "')");
    }

    public function update_db_config($site_name)
    {
        $this->db_query("UPDATE `" . $this->prefix . "config` SET `value` = '" . $this->db_escape($site_name) . "' WHERE `key` = 'config_site_title'");
    }

    public function update_notification_email($notification_email)
    {
        $this->db_query("UPDATE `" . $this->prefix . "config` SET `value` = '" . $this->db_escape($notification_email) . "' WHERE `key` = 'config_email'");
    }
