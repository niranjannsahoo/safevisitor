...
class State extends Admin_Controller {
	...
	protected function getListForm() {
		...
		$data['breadcrumbs'][] = array(
			'text' => "state",
			'href' => base_url('localisation/state')
		);
		/*form*/
		...
		if (!$this->uri->segment(4)) {
			$data['action'] = site_url("localisation/state/add");
		} else {
			$data['action'] = site_url("localisation/state/edit/".$this->uri->segment(4));
		}
		...
		$data['cancel'] = base_url('localisation/state');

		if ($this->uri->segment(4) && ($this->input->server('REQUEST_METHOD') != 'POST')) {
			$state_info = $this->state_model->getState($this->uri->segment(4));
		}
		...
		/*list*/
		$data['delete'] = base_url('localisation/state/delete');
		$data['datatable_url'] = base_url('localisation/state/search');
	...
	public function search() {
		...
		foreach($filteredData as $result) {
			...
			$action .= 		'<a class="btn btn-sm btn-primary" href="'.site_url('localisation/state/edit/'.$result->state_id).'"><i class="glyphicon glyphicon-pencil"></i></a>';
			$action .=		'<a class="btn-sm btn btn-danger btn-remove" href="'.base_url('localisation/state/delete/'.$result->state_id).'" onclick="return confirm(\'Are you sure?\') ? true : false;"><i class="glyphicon glyphicon-trash"></i></a>';
			$action .= '</div>';
			
			$datatable[]=array(
				'<input type="checkbox" name="selected[]" value="'.$result->state_id.'" />',
				$result->name,
				$result->code,
				$result->country,
